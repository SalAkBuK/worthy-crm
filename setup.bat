...
echo Applying notifications migration...
set "MIG_SQL_NOTIF=SET @tbl := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='%DB_NAME%' AND TABLE_NAME='notifications'); SET @sql := IF(@tbl=0, 'CREATE TABLE notifications (id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT, user_id INT UNSIGNED NOT NULL, type VARCHAR(60) NOT NULL, title VARCHAR(190) NOT NULL, body TEXT NULL, link_url VARCHAR(255) NULL, meta_json JSON NULL, dedup_key VARCHAR(190) NULL, read_at DATETIME NULL, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (id), KEY idx_notifications_user (user_id), KEY idx_notifications_read (read_at), KEY idx_notifications_created (created_at), UNIQUE KEY uq_notifications_dedup (user_id, dedup_key), CONSTRAINT fk_notifications_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE ON UPDATE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;', 'SELECT 1;'); PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;"
"%MYSQL_EXE%" %MYSQL_ARGS% "%DB_NAME%" -e "%MIG_SQL_NOTIF%"
